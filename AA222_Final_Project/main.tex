...
The rover dynamics and power models are adapted from an energy-constrained planetary rover formulation that couples rover motion with translational, rotational, resistive, and baseline subsystem power terms~\cite{hu2025rover}. A reduced-order version is used here: the rover is represented in planar coordinates, the optimizer commands speed and heading, and the power model maps commanded motion and terrain resistance to battery depletion.
...
\end{align}
Here $m$ is rover mass, $I_z$ is yaw inertia, $g$ is lunar gravity, $\phi$ is the local slope angle, and $c_0$ is an aggregate terrain-resistance coefficient. The numerical values used for these rover-model parameters, power parameters, battery limits, and trajectory constraints are summarized in Appendix~\ref{tab:rover_params}. The resistive term is a low-order surrogate for wheel--soil interaction losses. Detailed terramechanics models include compression, rolling, bulldozing, gravitational resistance, slip, sinkage, and shear effects~\cite{li2022terramechanics}; those effects are approximated here by $P_{\mathrm{res}}=c_0|v_B|$ to keep the trajectory optimization inexpensive.
...
The nonlinear OCP in Eq.~\eqref{eq:ocp} is implemented using sequential convex programming. At each SCP iteration, the rover dynamics, power constraint, and stage-energy expression are linearized about the current nominal trajectory $(\bar{x}_k,\bar{u}_k,\bar{T}_f)$. JAX \cite{jax2018github} is used to compute the required Jacobians, and CVXPY \cite{diamond2016cvxpy} is used to solve the convex subproblem.
...
Table~\ref{tab:risk_weight_sweep} shows the planner tradeoff more directly. Increasing $w_{\mathrm{risk}}$ increases the route length but decreases path-integrated risk, producing a clear distance--risk Pareto trend. This behavior is consistent with the planner edge cost, which combines geometric distance with a trapezoidal approximation of terrain-risk exposure. The agreement between the planner cost and the path-integrated risk metric provides an internal consistency check: the same risk field used to evaluate terrain exposure is also what drives the global route away from hazardous regions.
...
The trajectory-generation stage should be interpreted as a local feasibility and time--energy refinement stage around the planner route. The global terrain detour is primarily determined by A$^\ast$ through the risk-weighted terrain cost, while the SCP stage enforces the rover dynamics, power limits, battery bounds, rate constraints, and corridor constraints along the route. This decomposition is appropriate for the current implementation because it avoids requiring the local optimizer to solve the global obstacle-avoidance problem from scratch.
...
The time--energy objective is reflected in the long-duration, low-speed solution. However, the energy model is simplified: power demand is dominated by the baseline load and a low-order resistance term, while slip, sinkage, detailed wheel--soil interaction, active recharge, and spatially varying solar generation are not modeled explicitly. As a result, the nearly constant power trace should be interpreted as a feasibility-level energy estimate rather than a high-fidelity prediction of rover electrical load. The battery decrease is smaller than the integral of modeled power demand because the battery update subtracts the constant generation offset.
...
\section{Conclusion}
This project implemented a lunar rover navigation pipeline that combines terrain-aware global planning with SCP-based trajectory generation. The terrain module converts a LOLA-format elevation representation into traversability and risk fields, A$^\ast$ generates a feasible high-level route, and the trajectory-generation stage enforces rover dynamics, power, battery, rate, and corridor constraints along that route.

The benchmark result shows that the pipeline achieves the main navigation objective for the cratered terrain scenario. The planner avoids the hazardous crater-wall region, reducing path-integrated terrain risk by approximately $721\times$ relative to the straight-line baseline at a $1.66\times$ length cost. The returned trajectory covers approximately \SI{42.0}{km}, reaches the goal in approximately \SI{42.3}{h}, satisfies the nonlinear rover dynamics to numerical precision, and remains battery-feasible under the implemented energy model.

The main limitations are the synthetic benchmark, simplified energy model, and prototype local optimizer. The terrain does not include polar shadowing or illumination variation, and the energy model does not explicitly capture active recharge, spatially varying solar incidence, slip, sinkage, or detailed wheel--soil interaction. Future work should add terrain-risk or terrain-resistance terms directly to the local trajectory objective, improve the energy model, and evaluate the pipeline on polar LOLA-derived terrain for lunar south-pole mission planning~\cite{nasa_lola,moon_sokolov}.

\section{Contributions}
Martin Zhou implemented the terrain pipeline, global planning stack, and devised initial planner/SCP pipeline.

Sebastian Martinez implemented the rover dynamics model, the JAX-based SCP algorithm, and trajectory optimization tests.

Both authors contributed to the paper writing and revision.

\bibliography{sample}

\appendix
\section{Rover Model Parameters}

\begin{table}[h]
\centering
\caption{Rover model and trajectory-optimization parameters.}
\label{tab:rover_params}
\begin{tabular}{lll}
\toprule
Parameter & Value & Description \\
\midrule
$m$ & \SI{84.0}{kg} & Rover mass \\
$I_z$ & \SI{7.11}{kg.m^2} & Yaw moment of inertia \\
$g_\ell$ & \SI{1.62}{m/s^2} & Lunar gravity \\
$c_0$ & \SI{63.54}{N} & Aggregate terrain-resistance coefficient \\
$v_{\max}$ & \SI{0.45}{m/s} & Maximum commanded rover speed \\
$P_{\mathrm{base}}$ & \SI{100}{W} & Baseline subsystem power load \\
$P_{\mathrm{gen}}$ & \SI{100}{W} & Constant generation offset \\
$P_{\max}$ & \SI{200}{W} & Maximum modeled power consumption \\
$E_0$ & \SI{5.0}{MJ} & Initial battery energy \\
$E_{\min}$ & \SI{0}{J} & Minimum battery energy \\
$E_{\max}$ & \SI{5.0}{MJ} & Maximum battery energy \\
$a_{\max}$ & \SI{0.05}{m/s^2} & Acceleration limit \\
$\omega_{\max}$ & \SI{0.20}{rad/s} & Heading-rate limit \\
$\dot{\omega}_{\max}$ & \SI{0.05}{rad/s^2} & Yaw-acceleration limit \\
$r_{\mathrm{corr}}$ & \SI{100}{m} & Path-corridor radius \\
$c_E$ & $0.6$ & Energy weight in trajectory objective \\
$c_f$ & $0.4$ & Final-time weight in trajectory objective \\
$c_\nu$ & $0.1$ & Virtual-control penalty weight \\
\bottomrule
\end{tabular}
\end{table}
